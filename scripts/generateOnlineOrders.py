#!/usr/bin/python
# This script will generate Json data on demand from an online store for a fictitious robo Tailor Factory
#{
...
design = ["aqua", "boogie", "deft", "amazon", "google", "power", "velvet"]
sex = ["M", "F", "O"]
    ...
    for y in xrange(numberRuns):
        ...
        def create_names(fake):
            for x in range(numberRows):
                ...
                genDesign = design[random.randint(0,6)]
                genActivityTimestamp = strftime("%Y/%m/%d %H:%M:%S", gmtime())
                if x == 0:
                    file_object.write('[')

                file_object.write('{"userid": "' + genUserID + '","gender": "' + genSex + '","size": "' + genSize + '","material": "' + genMaterial + '","design": "' + genDesign + '","activityTimestamp": "' + genActivityTimestamp + '"}\n')
