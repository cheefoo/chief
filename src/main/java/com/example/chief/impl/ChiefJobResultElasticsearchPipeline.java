...
public class ChiefJobResultElasticsearchPipeline implements IChiefConnectorPipeline<JobResult, byte[]>  {
	
    @Override
    public IChiefEmitter<byte[]> getEmitter(ChiefConfiguration configuration) {
        return new ChiefJobResultElasticsearchEmitter(configuration);
    }
